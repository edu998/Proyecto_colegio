...
class Nota
{
    ...
    public function save()
    {
        ...
        if ($e_query && $e_query->num_rows != 0) {
            ...
            $sql = "CALL Add_nota ({$this->getUsuario_id()},{$this->getEstudiante_id()}, '{$this->getMateria_id()}', '{$this->getPrimeraNota()}', '{$this->getSegundaNota()}', '{$this->getTerceraNota()}', '{$this->getCuartaNota()}')";
    ...
    public function getAll($buscador = null)
    {
        ...
        if($buscador != null){
        $sql = "SELECT n.id AS 'id', e.cedula AS 'cedula', CONCAT(e.primer_nombre, ' ', e.primer_apellido) AS 'estudiante', m.nombre AS 'materia', n.primera_nota AS 'primera', n.segunda_nota AS 'segunda', n.tercera_nota AS 'tercera', n.cuarta_nota AS 'cuarta', (n.primera_nota + n.segunda_nota + n.tercera_nota + n.cuarta_nota)/4 AS 'nota_final' FROM nota n INNER JOIN usuario u ON n.usuario_id = u.id INNER JOIN estudiante e ON n.estudiante_id = e.id INNER JOIN materia m ON n.materia_id = m.id WHERE n.usuario_id={$this->getUsuario_id()} AND m.nombre LIKE '$buscador'";
        }
